...
export function SuggestionBubble({ suggestion, onReject, onAccept, isLeft }: SuggestionBubbleProps) {
  ...
  const style: React.CSSProperties = isManager && suggestion.x !== undefined ? {
    left: `${suggestion.x}%`,
    top: `${suggestion.y}%`,
    bottom: 'auto',
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <img src={avatar} alt="Char" className={`w-32 h-32 md:w-48 md:h-48 object-contain drop-shadow-2xl ${isManager ? "animate-bounce" : ""}`} />
        {isManager && (
          <div className="absolute top-2 right-2 bg-red-600 text-white text-[10px] px-2 py-0.5 rounded-full font-black rotate-12 shadow-md">BOSS</div>
        )}
      </div>
    </motion.div>
  );
